<template>
    <element-border :height="260">
        <p class="p-component">FC电流-电压</p>
        <el-row>
            <el-col style="height: 220px" id="VehicleSecondFCChart"></el-col>
        </el-row>
    </element-border>
</template>
...
import {wsFCV} from "@/api/consumer-ws";
import {JSONData} from "@/api/websocket-util";

export default {
    name: "VehicleSecondFCChart",
    data() {
        return {
            ws: null,
    ...
    methods: {
        initWs() {
            this.ws = this.$ws(wsFCV(this.id))
        },
        drawLine() {
            let chart = this.$chart.init(document.getElementById("VehicleSecondFCChart"));
