<template>
    <element-border animation="top" class="alarm-border" :height="230">
        <p class="p-component">报警状态</p>
        <div v-show="getterAlarmDataByFill.length > 0">
            <el-scrollbar height="180px" :always="false" ref="srcTransport">
                <div ref="scrContentBoxTransport" @mouseover="pauseScr()" @mouseout="startScroll()">
                    <el-timeline>
                        <el-timeline-item
                            v-for="item in getterAlarmDataByFill"
                            :key="item.id"
                            color="white"
...
export default {
    ...
    computed: {
        ...mapGetters(["getterAlarmDataByFill"]),
    },
    unmounted() {
        this.clearInterval();
    }
